...
async function handle(os: AgentOS, tm: TerminalManager, autos: Automations, req: http.IncomingMessage, res: http.ServerResponse, ttydPort?: number, slack?: SlackSocket, discord?: DiscordSocket, appSup?: AppSupervisor, clickup?: ClickupIngress, telegram?: TelegramSocket): Promise<void> {
  ...
  if (method === 'POST' && p === '/api/tasks/update') {
    ...
    const task = requestMetrics.phase('task:update.store', () => os.tasks.update(id, {
      ...
      // What a `blocked` task waits on — decides whether the delegating agent is woken or only the owner
      // is carded. Validated against the vocabulary here; the store clears it when the task leaves blocked.
